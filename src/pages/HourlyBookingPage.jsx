...
const HourlyBookingPage = () => {
  ...
  useEffect(() => {
    const loadAddons = async () => {
      try {
        const addons = await fetchAddonTemplates();
        // Filter only active addons
        setAvailableAddons(addons?.filter(a => a.is_active !== false) || []);
      } catch (error) {
  ...
  // Restore saved booking data after login (e.g., Google OAuth redirect)
  useEffect(() => {
    const savedData = sessionStorage.getItem('hourlyBookingFormData');
    if (savedData && user) {
      try {
        const formData = JSON.parse(savedData);
        if (formData.serviceType) setServiceType(formData.serviceType);
        if (formData.dateTime) setDateTime(formData.dateTime);
        if (formData.cleaners) setCleaners(formData.cleaners);
        if (formData.hours) setHours(formData.hours);
        if (formData.workCondition) setWorkCondition(formData.workCondition);
        
        // Clear saved data after restoring
        sessionStorage.removeItem('hourlyBookingFormData');
        
        // Auto-advance to step 2 if data was restored
  ...
  const saveBookingDetails = () => {
    ...
    sessionStorage.setItem('hourlyBookingFormData', JSON.stringify(formData));
  };
  ...
  const handleGoogleLogin = async () => {
    ...
    const { data, error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/hourlybooking`
      }
    });
    if (error) {
      toast({
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 md:py-12">
        ...
        <div className="mb-8 flex items-center justify-center max-w-2xl mx-auto">
          ...
          <div className="flex items-center w-full">
            ...
            <div className={cn("flex-1 h-1 mx-2 transition-colors", currentStep >= 3 ? "bg-primary" : "bg-muted")} />

            <div className={cn("flex items-center justify-center w-10 h-10 rounded-full font-bold transition-colors", currentStep >= 3 ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground")}>3</div>
        ...
        {currentStep === 2 && (
          ...
          <div className="max-w-3xl mx-auto space-y-8 animate-in slide-in-from-right-4 fade-in duration-300">
            ...
            <Button 
              ...
              className="w-full py-6 text-lg shadow-lg bg-primary hover:bg-primary/90 text-primary-foreground dark:bg-sky-500 dark:hover:bg-sky-600 font-bold tracking-wide transition-all active:scale-[0.98]"
