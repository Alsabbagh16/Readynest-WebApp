...
begin
  ...
  ranked as (
    ...
    where lower(trim(coalesce(employees.position, ''))) in ('cleaner', 'part timer', 'part-time', 'part time')
  ...
  select coalesce(jsonb_agg(
    jsonb_build_object(
      ...
      'employee_name', ranked.employee_name,
      'email', ranked.email,
      'is_part_timer', ranked.is_part_timer,
      'jobs_completed', ranked.jobs_completed,
      'hours_worked', ranked.hours_worked
    )
